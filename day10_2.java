...
The original prices of the photo-frames are given as C[], 
determine the minimum cost to purchase all N photo-frames.

For example, if there are K=3 friends that want to buy N=4, photo-frames that cost C=[1,2,3,4]
each will buy one of the photo-frames priced [2,3,4] at the original price. 
Having each purchased 1 photo-frame, the price of the photo-frame-1 is c[0] , now it will cost 
...
Explanation:
----------------
The friends buy photo-frames for 9, 7 and 3, 5 and 1 for a cost of 9+ 7+3*(1+1) + 5+1*(1+1).
 */
...
class day10_2
{
    public static void main(String args[])
    {
        ...
        int counter=0,ans=0,sold=0;
        while(!pq.isEmpty())
        {
            ans+=(counter+1)*pq.poll();
            sold++;
            if(sold%k==0)
            counter++;
        }
